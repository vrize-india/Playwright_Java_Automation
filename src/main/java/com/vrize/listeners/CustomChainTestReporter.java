...
public class CustomChainTestReporter implements IReporter {
    ...
    public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites, String outputDirectory) {
        ...
        System.out.println("[CustomChainTestReporter] Xray enabled: " + com.vrize.listeners.XrayLogger.isXrayEnabled());
        ...
        try {
            ...
            sb.append("<input type='text' id='tag-search-non-vrize' class='form-control mb-2' placeholder='Search tags...'>");
            sb.append("<table id='tag-summary-non-vrize' class='table table-striped display'><thead><tr>");
            sb.append("<th>Tag</th>");
            ...
            sb.append("<script>\n" +
                    "$(document).ready(function() {\n" +
                    "  $('#tag-summary-non-vrize').DataTable({\n" +
                    "    paging: false,\n" +
                    "    info: false,\n" +
                    "    searching: true,\n" +
                    "    ordering: true\n" +
            ...
            System.out.println("[Custom Tag Summary] tag-summary-non-vrize table generated with " + nonTonicRows.size() + " rows.");
        } catch (Exception e) {
            System.err.println("[Custom Tag Summary] Error generating tag-summary-non-vrize table: " + e.getMessage());
        }
        
        // Smart report attachment logic that works for both single-run and retry scenarios
        try {
            if (com.vrize.listeners.XrayLogger.isXrayEnabled()) {
                ...
                if (isSingleRun) {
                    ...
                    com.vrize.listeners.XrayLogger.attachReportToTestExecution("target/chaintest/Index.html");
